...
I added a `--first_note` flag. Documented it lovingly in argparse ("a number between 1-8"). Threaded
it through `main()`. Stamped it into the filename of every single file I generated. And then I looped
`range(first_note, 0)`.
...
Both 232 bytes. They differ at **exactly one byte** — byte 43 — which is the digit I wrote into the
track name. The music is identical. Six of every seven "major" files in that corpus are the same song
...
**2,028** files instead of 14,196 (14,196 ÷ 7 = 2,028, exactly). The same bug quietly *cloned* my
melodies and quietly *deleted* my drums. Perfect symmetry. Chef's kiss.
...
use `midiutil`, a third-party library, correctly — so I clearly knew libraries existed. `zeroes()` is
a small monument to a Tuesday.
...
semitone partners `16, 23`. It looks like real theory. But four lines into `next_step` I fold every
interval back below 12 *before* I ever test membership. So those upper entries can **never match**.
They are painted-on windows. Half the music-theory brain is a beautiful, load-bearing-looking diagram
of a door that isn't there.

Bonus, two lines up: `self.tones = ''`. Not `[]`. An empty **string**, initialised and standing by,
in case I ever wanted to `.append` to it and find out live what a `TypeError` feels like.

---

## Exhibit D — `noise.py`, the engine I smoothed to death
...
A Savitzky–Golay filter smooths a signal. I ran it four times in a row, which is how you arrive at a
melody with the dynamic range of a dial tone. A few lines down:
...
out of bounds and 15 made the crash stop — followed by a `print` I used to watch it and then left in
